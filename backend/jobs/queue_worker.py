...
from backend.jobs.dispatch import get_handler
...
def _mark_running(db: Session, job: BackgroundJob, attempts: int, locked_by: str) -> None:
...
def _handle_job(
    ...
    try:
        ...
        if job.status in {"succeeded", "dlq"}:
            queue.ack(entry_id, redis_client=redis_client)
            return
        if job.not_before_at and datetime.now(timezone.utc) < job.not_before_at:
            record_event(
                db,
                job.job_id,
                "deferred",
                data_json={"not_before_at": job.not_before_at.isoformat()},
            )
            return

        for attempt in range(job.attempts + 1, MAX_ATTEMPTS + 1):
            _mark_running(db, job, attempt, consumer)
            try:
                handler = get_handler(job_type)
                result = handler(payload, headers, db)
...
def process_once(
    ...
    for entry_id, fields in queue.claim_stuck(
        ...
        _handle_job(entry_id, fields, redis_client, db_session_factory, consumer)
        processed += 1
