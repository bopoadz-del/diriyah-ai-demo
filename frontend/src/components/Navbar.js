...
const Navbar = () => {
  ...
  const handleFileChange = useCallback(async (event) => {
    ...
    try {
      ...
      window.alert(`File uploaded successfully. Result: ${JSON.stringify(payload.result)}`);
    } catch (uploadError) {
      console.error("Failed to upload file", uploadError); setError("Unable to upload file. Please try again.");
    } finally {
  ...
  const toggleAlerts = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to fetch alerts", alertsError); setError("Unable to load alerts.");
    }
  }, [showAlerts]);
    ...
    <div className="navbar" role="navigation" aria-label="Primary">
      ...
      {showAlerts && (
        <div className="dropdown" role="status" aria-live="polite">
          {alerts.map((alert, index) => (<p key={`${alert.message}-${index}`} className={alert.level}>{alert.message}</p>))}
